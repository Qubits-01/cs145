...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="test0.py", description="Tests your Programming Task 1 (PT 1) solution."
    )
    ...
    parser.add_argument(
        ...
        "--num-tests",
        default=100,
        help="Number of tests to run (integer; default: 100).",
        type=int,
    )
    ...
    for i in range(num_tests_arg):
        ...
        # print(f"[test0.py] Running command: {command}")
    ...
    x = log2(
        total_bits if num_tests_arg == DEFAULT_NUM_TESTS else avg * DEFAULT_NUM_TESTS
    )
    if time_errors >= 3:
        score = 0
    elif x > 21.99:
